...
console.log('[DEBUG] This is the running index.js');
...
const options = {
  cert: fs.readFileSync('certbot-cert.pem'),
  key: fs.readFileSync('certbot-key.pem')
};
...
/*db.query('SHOW VARIABLES LIKE "character_set_connection"', function (error, results) {
  if (error) throw error;
  console.log(results[0].Value); // This will output the character set of the connection
});*/
...
app.post("/IMREAL/api/get_dao", (req, res) => {
  ...
  console.log("[get_dao] Incoming request body:", req.body);
  const { wallet } = req.body;

  // TODO: Implement admin check logic here if required
  // Example (pseudo):
  // if (!isAdmin(wallet)) {
  //   return res.status(403).json({ error: 'Not authorized' });
  // }

  const sql_get = "SELECT *, 1 AS owned FROM dao WHERE progress = 6";
  db.query(sql_get, (err, result) => {
    if (err) {
      console.log("[get_dao] DB error:", err);
      res.status(500).json({ error: 'Database error', details: err });
    } else {
      console.log("[get_dao] Query result count:", result.length);
      res.send(result);
    }
  });
});
...
app.delete("/IMREAL/api/removeAllNotifications/:wallet", (req, res) => {
  const wallet = req.params.wallet;
  const sql_update = "UPDATE notifications SET notif_status =2 WHERE notif_for_wallet =?";
  db.query(sql_update, wallet, (err, result) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'Database error', details: err });
    } else {
...
app.post("/IMREAL/api/removeNotification", (req, res) => {
  const wallet = req.body.wallet;
  const notifID = req.body.notifID;
  const sql_get = "UPDATE notifications SET notif_status =2 WHERE notif_for_wallet =? AND id =?";
  db.query(sql_get, [wallet, notifID], (err, result) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'Database error', details: err });
    } else {
...
app.post("/IMREAL/api/getNotifications", (req, res) => {
  const wallet = req.body.wallet;
  const sql_get = "SELECT * FROM notifications WHERE notif_for_wallet =? AND notif_status =0 ORDER BY id DESC";
  db.query(sql_get, wallet, (err, result) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'Database error', details: err });
...
app.post("/IMREAL/api/getAllNotifications", (req, res) => {
  const wallet = req.body.wallet;
  const sql_get = "SELECT * FROM notifications WHERE notif_for_wallet =? AND notif_status <= 1 ORDER BY notif_status,id DESC";
  console.log('[DEBUG] /getAllNotifications called with wallet:', wallet);
  console.log('[DEBUG] SQL Query:', sql_get, '| Params:', wallet);
  db.query(sql_get, wallet, (err, result) => {
    if (err) {
      console.error('[DEBUG] DB error in /getAllNotifications:', err);
      return res.status(500).json({ error: 'Database error', details: err });
    } else {
      console.log('[DEBUG] /getAllNotifications result:', result);
      res.send(result);
    }
  });
});

app.post("/IMREAL/api/notifClicked", (req, res) => {
  const notifID = req.body.notifID;
  const sql_update = "UPDATE notifications SET notif_status =1 WHERE id =?";
  db.query(sql_update, notifID, (err, result) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'Database error', details: err });
    } else {
...
app.post("/IMREAL/api/notifUpdate", (req, res) => {
  const notifID = req.body.notifID;
  const notifStatus = req.body.notifStatus;
  const sql_update = "UPDATE notifications SET notif_status =1 WHERE id =?";
  if (notifStatus === 0) {
    db.query(sql_update, notifID, (err, result) => {
      if (err) {
...
app.post("/IMREAL/api/notifClear", (req, res) => {
  const wallet = req.body.wallet;
  const sql_update = "UPDATE notifications SET notif_status =1 WHERE notif_for_wallet =?";
  db.query(sql_update, wallet, (err, result) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'Database error', details: err });
    } else {
      console.log('[DEBUG] /IMREAL/api/notifClear success:', result);
      res.send(result);
    }
  });
});
...
const addNewUser = async (wallet, socketID) => {
  if (wallet) {
    const res = await axios.get('https://geolocation-db.com/json/');
    const sql_select = "SELECT * FROM admin where wallet =?";
    db.query(sql_select, wallet, async (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (result[0]) {
          const isAdmin = 1;
          !onlineUsers.some((user) => user.wallet === wallet) &&
            onlineUsers.push({ wallet, socketID, isAdmin, ...result[0], ...res.data });
...
const getUser = (wallet) => {
  return onlineUsers.find((user) => user.wallet === wallet);
};

io.on("connection", (socket) => {
  console.log(`Connected! ${socket.id}`);

  socket.on("newUser", (wallet) => {
    addNewUser(wallet, socket.id);
  });

  socket.on("inviteMember", ({ kind, inviteeAddress, inviter, daoName, daoLink, daoIMG }) => {
    const receiver = getUser(inviteeAddress);
    const notifShortMSG = `Invitation from ${daoName}.`;
    const notifLongMSG = `${inviter} invited you to join their DAO`;
    const dateTimeNow = new Date();

    const mysql_query = "INSERT INTO notifications (notif_kind,notif_name,notif_for_wallet,notif_img,notif_short_message,notif_long_message,notif_link,notif_status,notif_created) VALUES (?,?,?,?,?,?,?,0,Now())";
    db.query(mysql_query, [kind, daoName, inviteeAddress, daoIMG, notifShortMSG, notifLongMSG, daoLink], (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (receiver) {
          io.to(receiver.socketID).emit("getInviteMember", {
            kind
          });
        }
      }
  ...
  socket.on("proposed", ({ kind, proposer, daoName, daoLink, daoIMG, editionDrop }) => {
    const notifShortMSG = `New proposal from ${daoName}.`;
    const notifLongMSG = `${proposer} has proposed a new proposal`;
    const dateTimeNow = new Date();

    const mysql_query = "SELECT * FROM claimers WHERE editionDrop_address =? AND wallet !=? AND status =1";
    const mysql_insert = "INSERT INTO notifications (notif_kind,notif_name,notif_for_wallet,notif_img,notif_short_message,notif_long_message,notif_link,notif_status,notif_created) VALUES (?,?,?,?,?,?,?,0,Now())";
    db.query(mysql_query, [editionDrop, proposer], (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (result) {
          for (let i = 0; i < result.length; i++) {
            const receiver = getUser(result[i]?.wallet);
            db.query(mysql_insert, [kind, daoName, result[i].wallet, daoIMG, notifShortMSG, notifLongMSG, daoLink], (err, result) => {
              if (err) {
                console.log(err);
              } else {
                if (receiver) {
                  io.to(receiver.socketID).emit("proposalNotif", {
                    kind
                  });
                }
              }
  ...
  socket.on("submitVote", ({ kind, voter, daoName, daoLink, daoIMG, editionDrop }) => {
    const notifShortMSG = "A new vote on a proposal";
    const notifLongMSG = `${voter} has voted to the proposal.`;

    const mysql_query = "SELECT * FROM claimers WHERE editionDrop_address =? AND wallet !=? AND status =1";
    const mysql_insert = "INSERT INTO notifications (notif_kind,notif_name,notif_for_wallet,notif_img,notif_short_message,notif_long_message,notif_link,notif_status,notif_created) VALUES (?,?,?,?,?,?,?,0,Now())";

    db.query(mysql_query, [editionDrop, voter], (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (result) {
  ...
  socket.on("commented", ({ kind, commenter, daoName, daoLink, daoIMG, editionDrop }) => {
    const notifShortMSG = "New proposal comment";
    const notifLongMSG = `${commenter} commented on a proposal you were in.`;

    const mysql_query = "SELECT * FROM claimers WHERE editionDrop_address =? AND wallet !=? AND status =1";
    const mysql_insert = "INSERT INTO notifications (notif_kind,notif_name,notif_for_wallet,notif_img,notif_short_message,notif_long_message,notif_link,notif_status,notif_created) VALUES (?,?,?,?,?,?,?,0,Now())";

    db.query(mysql_query, [editionDrop, commenter], (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (result) {
  ...
  socket.on("request", ({ kind, requester, daoName, daoIMG }) => {
    const notifShortMSG = "New deactivation request";
    const notifLongMSG = `${requester} requested to deactivate the DAO.`;
    const adminLink = "/adminpanel?tab=request";

    const mysql_query = "SELECT * FROM admin WHERE requests =1 AND isActive =1";
    const mysql_insert = "INSERT INTO notifications (notif_kind,notif_name,notif_for_wallet,notif_img,notif_short_message,notif_long_message,notif_link,notif_status,notif_created) VALUES (?,?,?,?,?,?,?,0,Now())";

    db.query(mysql_query, (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (result) {
          for (let i = 0; i < result.length; i++) {
            const receiver = getUser(result[i]?.wallet);
            db.query(mysql_insert, [kind, daoName, result[i].wallet, daoIMG, notifShortMSG, notifLongMSG, adminLink], (err, result) => {
              if (err) {
                console.log(err);
              } else {
                if (receiver) {
  ...
  socket.on("newMember", ({ kind, wallet, editionDrop, owner, daoName, daoLink, daoIMG }) => {
    const notifShortMSG = "New DAO member";
    const notifLongMSG = `${wallet} has joined our DAO!`;

    const mysql_query = "SELECT * FROM claimers WHERE editionDrop_address =? AND status =1";
    const mysql_insert = "INSERT INTO notifications (notif_kind,notif_name,notif_for_wallet,notif_img,notif_short_message,notif_long_message,notif_link,notif_status,notif_created) VALUES (?,?,?,?,?,?,?,0,Now())";
    db.query(mysql_query, editionDrop, (err, result) => {
      if (err) {
        console.log(err);
      } else {
        if (result) {
  ...
  socket.on("openedNotif", ({ wallet }) => {
    const receiver = getUser(wallet);
    if (receiver) {
      io.to(receiver.socketID).emit("proposalNotif", {
        wallet
      });
  ...
  io.emit("firstEvent", "This is a first test event!");
});

// MISSING BLOCK CLOSURE FIXED
